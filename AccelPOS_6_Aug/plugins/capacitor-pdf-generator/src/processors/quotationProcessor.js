...
export class QuotationProcessor {
  ...
  async processIllustrationInput(requestJSON, planName) {
    ...
    console.log('🔍 Normalized names:', { normalizedPlanName, normalizedProductName });
    ...
    console.log('🔍 Template keys:', { templateKey, configKey });

    // Load template HTML
    console.log('🔍 Loading template HTML for key:', templateKey);
    const templateHtml = this.templateManager.bundledAssets.get(templateKey);
    console.log('🔍 Template HTML loaded:', templateHtml ? 'success' : 'failed');
    if (!templateHtml) {
      throw new Error(`Template not found: ${templateKey}`);
    }

    // Load config JSON
    console.log('🔍 Loading config JSON from:', `assets/templates/${configKey}/TableFormat.json`);
