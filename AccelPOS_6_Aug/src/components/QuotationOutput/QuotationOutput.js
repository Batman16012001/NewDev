...
import capacitorPdfGenerator from '../../plugins/capacitor-pdf-generator/dist/index.js';

console.log('🔍 Plugin import check:', capacitorPdfGenerator);
console.log('🔍 Plugin methods:', Object.keys(capacitorPdfGenerator || {}));
...
const QuotationOutput = () => {
    ...
    const OutputAPI = async (sqsdata, riderdata, personaldata) => {
        ...
        console.log("Pranav Shinde Hello::::")
        if (!status.connected) {
            console.log('OFFLINE: Using Capacitor PDF Generator Plugin');
            // OFFLINE: Use plugin for PDF generation
            let planName = sqsrealdata.policyDetails.productName.replace(/\s+/g, '-').toLowerCase();

            try {
                console.log('🔍 About to initialize plugin...');
                // Initialize plugin
                ...
                console.log('🔍 generatePDF called successfully, result:', result);

                if (result.success) {
                    // Handle different data types from plugin
                    let pdfBlobUrl;
                    if (result.data instanceof Blob) {
                        // Data is a Blob, create blob URL
                        pdfBlobUrl = URL.createObjectURL(result.data);
                    } else if (typeof result.data === 'string' && result.data.startsWith('blob:')) {
                        // Data is already a blob URL
                        pdfBlobUrl = result.data;
                    } else {
                        console.error('Unexpected data type from plugin:', typeof result.data);
                        throw new Error('Invalid data type from plugin');
                    }
                    console.log('Plugin generated PDF URL:', pdfBlobUrl);
                    return pdfBlobUrl;
                } else {
                    console.error('Plugin processing failed:', result.error);
            ...
            // ONLINE: Use plugin (as per hybrid strategy - web always uses backend, mobile uses backend when online)
            const isMobile = /Android|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent);
            let planName = sqsrealdata.policyDetails.productName.replace(/\s+/g, '-').toLowerCase();
            
            if (isMobile) {
                // Mobile online: Use plugin (which will call backend internally)
                console.log('ONLINE MOBILE: Using Capacitor PDF Generator Plugin');
                try {
                    // Initialize plugin
                    await capacitorPdfGenerator.initialize();

                    // Generate PDF using plugin (will call backend internally)
                    const result = await capacitorPdfGenerator.generatePDF({
                        outputType: 'quotation',
                        inputData: quotedata,
                        template: planName
                ...
                // Web online: Call backend directly
                console.log('ONLINE WEB: Calling backend API directly');
        try {
