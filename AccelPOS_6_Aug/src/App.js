...
import Products from "./components/Dashboard/Products";
import NeedsCalculator from "./components/NeedsCalculator/NeedsCalculator";
// import { preloadOfflineData } from "./utils/Offline/preloadOfflineData";

import JSZip from "jszip";
import { Filesystem, Directory, Encoding } from "@capacitor/filesystem";
import axios from "axios";

// import { QuestionnairePlugin } from "accelquestionnaire";

// QuestionnairePlugin.downloadStaticJson().then((result) => {
//   console.log("JSON downloaded", result);
// });

// QuestionnairePlugin.downloadAndStoreQuestions()
//   .then((result) => {
//     console.log(" Plugin working! Result:", result);
//   })
//   .catch((err) => {
...
const App = () => {
  ...
  useEffect(() => {
    const downloadAndStoreZip = async () => {
      try {
        const url =
          "http://192.168.2.52:3003/leadFormManagement/getStaticQuestionnaireData";
        ...
        for (const filename of fileNames) {
          ...
          if (file.dir) {
            ...
            // It's a file — make sure parent folders exist
            const fileData = await file.async("string");

            const dirPath = `questionnaire/${filename.substring(
              0,
              filename.lastIndexOf("/")
            )}`;

            if (dirPath && dirPath !== "questionnaire") {
              await Filesystem.mkdir({
                path: dirPath,
                directory: Directory.Data,
                recursive: true,
        ...
        console.error("Error downloading or saving zip:", err);
      }
    };
  ...
  return (
    <Router>
      <Routes>
        ...
        <Route path="/fnasignature" element={<FNA_Signature />} />
        <Route
          path="/fnaproducrrecomnedations"
          element={<FNA_ProductRecomendations />}
        />
        <Route path="/fnaneeds" element={<FNA_AllNeedsGraphs />} />
        <Route path="/fnasummary" element={<FNA_Summary />} />
        <Route path="/fnareport" element={<FNA_Output />} />
