...
const QuotationTemplate = ({ pdfBlobUrl }) => {
  useEffect(() => {
    console.log("Recieved ::: ", pdfBlobUrl)
    ...
    //             path: folderName, // Just provide the folder name; it will be created inside Documents
    //             directory: Directory.Documents, // Use Documents directory
    //             recursive: true,
    //           });
    //         } catch (error) {
    //           if (error.message !== 'Directory exists') {
    //             console.error('Error creating directory:', error);
    //             return; // Exit if it's not a "Directory exists" error
    //           }
    //         }

    //         // Write the PDF to the subfolder
    //         await Filesystem.writeFile({
    //           path: `${folderName}/${fileName}`, // Save file in the subfolder
    //           data: base64data,
    //           directory: Directory.Documents, // Use Documents directory
    //           recursive: true,
    //         });
  ...
  return (
    <div >
      <h5 className='text-center'>Quotation Pdf Viewer</h5>
      {pdfBlobUrl ? (
